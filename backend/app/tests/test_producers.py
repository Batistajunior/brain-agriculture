...
def test_create_producer(client):
    payload = {"cpf_cnpj": "12345678901", "name": "Produtor Teste"}
    response = client.post("/producers/", json=payload)
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "Produtor Teste"
    assert data["cpf_cnpj"] == "12345678901"


def test_list_producers(client):
    # cria um produtor
    client.post("/producers/", json={"cpf_cnpj": "12345678901", "name": "Produtor Teste"})
    # lista produtores
    response = client.get("/producers/")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["name"] == "Produtor Teste"
